#!/usr/bin/env python3
"""Load sampling-clock pins from swarm ledgers. No invented glutamate kon/koff."""
...
# Pins to the current ledger snapshot. If a number moves, tests fail.
# These are not new measurements.
PINNED = {
    "E033": ("1.2", "ms"),  # Clements τ (stored as response_time)
    "E032": ("1.1", "mM"),  # Clements inferred peak
    "E034": ("25", "nM"),  # Herman ambient; NOT spliced into the plotted waveform
...
def load_clocks(
    evidence_csv: Path | None = None, claims_csv: Path | None = None
) -> LedgerClocks:
    ...
    if "C007" not in claims:
        raise KeyError("missing C007")
    ...
    c007 = claims["C007"]["claim_text"]
    if "kon" not in c007.lower() or "koff" not in c007.lower():
        raise ValueError("C007 no longer names empty glutamate kon/koff")
    ...
    interrogation_s = _to_seconds(
        _pinned_value(evidence, "E045", "numerical_result"), PINNED["E045"][1]
    )
    sampling_s = _to_seconds(
        _pinned_value(evidence, "E046", "numerical_result"), PINNED["E046"][1]
    )
    incubation_s = _to_seconds(
        _pinned_value(evidence, "E008", "numerical_result"), PINNED["E008"][1]
    )
    plateau_s = _to_seconds(
        _pinned_value(evidence, "E042", "numerical_result"), PINNED["E042"][1]
    )
