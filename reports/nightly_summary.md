...
**Demoted as flagship:** 81-fold vs ~44,000-fold. The 81-fold identity (C027) stays as a two-line derivation. The comparison fails as a load-bearing glyph because the basal pole is method-dependent; parked PRs #18/#26/#31 document other primary basal measurements that are **not** yet `claims.csv` rows on `main` and must not be averaged with C012. Demote `occupancy.svg` as a tissue-occupancy finding (SIMULATION of advertised Kd/EC50, device hop).
