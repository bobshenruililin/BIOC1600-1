...
1. Hu 2025 RWTH thesis S066 DOI 10.18154/RWTH-2025-07238 — in vitro mouse retina; 14 s / 1 min ACV; two ACV legs; authors interpret as basal/sustained, not transients; not a pharmacological ID.
...
1. **Recognition ≠ architecture ≠ biological process** until a cited experiment connects them. Do not transfer 1.8 nM onto PaC/tissue occupancy.
2. **Hu 39-mer** is the exact 5′ of the Lam-quoted 98-nt Wu string. **12 µM is a citation hop**, not a 39-mer Kd.
3. **S066 is the only nervous-tissue Glu-apt experiment in the ledger, and its Glu identity is untested on both ACV legs**, including Hu’s baseline-rise “successful detection” sentence.
4. **No THE basal spec.** Hascup 2008 Str 5.0 ± 1.2 µM is a real GluOx resting, not the Hu-class pole. Ames contains both 25 nM and 5 µM. Do not pair 5 µM with Clements.
...
1. Canonical Claim 5 / Panel 3 previously named identified glutamate while the same files listed identity as UNKNOWN. **This INT retitles both legs.**
...
3. Isolated LF-*n* algebra `81^(1/n)` is correct, but that package still draws the withdrawn 44000-fold overlay. Measuring *n* does not restore the flagship. Accepted `occupancy_kinetics` on `main` is 1:1 Langmuir only.
...
A two-panel glyph of printed PBS vs Ames slopes is optional supporting analysis only. It is **not** a path to 85+.
...
See `poster/storyboards/revised_D.md`. Provisional panels: two regimes; Mission-2 flagship placeholder; Hu retina two ACV legs on a slow clock, identity UNKNOWN; PaC occupancy UNKNOWN; construct atlas; proposed next measurement.
...
1. What did Hu measure in the retina, and what would make you stop calling it glutamate — including the post-insertion baseline-rise sentence?
2. Why is 1.8 nM not the PaC Kd, and why is 12 µM not the 39-mer Kd?
...
5. Why doesn’t averaging 25 nM with 5 µM give “the” basal?
...
Parent of this INT: `65971ae19714a783013e22a150cc474d26e9295c` (`origin/main`, merge of PR 28). This branch: `cursor/m2-int-honesty-245a`. Isolated analysis SHAs in §6. Do not use overnight `984af62` or pre-reset `c87ebe4` as current science.
...
GROUP FINAL: NO
HUMAN DECISION REQUIRED: Lock U2 vs U1; do not set PASS or group-final without a measured identity or construct-locked quantity on this film. 85+ HONEST: NO.
```
