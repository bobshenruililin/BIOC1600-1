...
This file now carries that recommendation in §1. Flagship after Wave D (PR #31): A4 interrogation Nyquist, not 81-vs-44000. It does **not** convert the gate to PASS. Human approval still required for group-final.
...
Status: **recommended working thesis, not group-final** (`D_revised`). Gate remains **REVISE**. Herman ~25 nM is one method-class basal (C012), not a retinal range. Do not treat a single Herman-to-Clements span as settled biology. Hu 1.8 nM is an AuED-MEA Langmuir–Freundlich apparent Kd, not the PaC probe’s molecular Kd.
...
3. **One 1:1 Langmuir site has an 81-fold 10–90% window** (derived, C027). That identity is not a fitted window for Hu’s Langmuir–Freundlich sensor. Comparing it to a single Herman-to-Clements span is **not** the flagship (Wave D, PR #31). Dual-compartment sensing is not a single-Kd job.
...
**A4 interrogation Nyquist** (PR #24, `analysis/accepted/interrogation_nyquist/`): measured PaC-probe interrogation and sampling clocks (C031, C028; 14 s/ACV, ~1 min/point) versus a labelled 1.2 ms literature stimulus (C011). Public-data feasible: stdlib Python, no network, values pinned to existing `core_evidence.csv` rows. No glutamate `kon`/`koff` is filled. Do not caption “aptamer too slow.” Clements τ remains a cultured-hippocampal inference, not a retinal measurement. If the figure draws a 25 nM pulse floor, that floor is a literature stimulus, not a retinal spec.

**Demoted as flagship:** 81-fold vs ~44,000-fold. The 81-fold identity (C027) stays as a two-line derivation. For Clements 1.1 mM (C011), 1.1 mM / 81 ≈ 13.6 µM is the basal at which that window covers the span (`computational illustration` on C011 and C027 only). The comparison fails as a load-bearing glyph because which side of 13.6 µM biology sits on is method-dependent; parked PRs #18/#26/#31 document other primary basal measurements that are **not** yet `claims.csv` rows on `main` and must not be averaged with C012. Demote `occupancy.svg` as a tissue-occupancy finding (SIMULATION of advertised Kd/EC50, device hop).

Supporting: evidence atlas (`atlas.svg`); `clocks.svg`; `sensitivity.svg` (NOT glutamate). A3 working-range bars (PR #23) are a supporting panel once the left tick is a basal *band*, not a 25 nM point.
...
| 2 | `analysis/accepted/figures/clocks.svg` | Same-construct clocks that A4 uses; 14 s / 1 min vs literature 1.2 ms | MEASURED vs INFERENCE |
| 3 | `analysis/accepted/figures/occupancy.svg` | 81-fold identity lives here; θ overlay is SIMULATION not tissue occupancy | SIMULATION; nM/mM ticks |
