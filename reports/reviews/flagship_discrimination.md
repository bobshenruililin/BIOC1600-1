...
- Hascup 2008 Table 1, read in PMC HTML: "PFC (n = 8) … Average 3.3 ± 1.0 … Str (n = 10) … Average 5.0 ± 1.2", µM, days 3–7 post-implant, and the authors note no significant difference between regions (p = 0.31).

Basal-pole spread across these four: 34.7 µM / 25 nM = **1,388-fold**.

**On "construct-faithful."** These are biological concentrations, so there is no aptamer construct to be faithful to; the applicable hygiene is system fidelity, and I have kept it. Every row above carries its method, species, region, probe class, estimator and n; none was averaged with another; none was transferred onto a sensor property. PR #26 states the same prohibition explicitly for these exact rows: "Herman ~25 nM … Different method/preparation; do not 'reconcile' by averaging with 9.4 or 34.7." My argument does not require any of them to be transferred — only that they exist and disagree.
...
Then I checked what instrument produced the *other* pole, and it is the same one. Clements 1992's abstract, which I re-read this pass (PMID 1359647): "The time course of free glutamate in the cleft was estimated by kinetic analysis of the displacement of a rapidly dissociating competitive antagonist from N-methyl-D-aspartate (NMDA) receptors during synaptic transmission. Glutamate peaked at 1.1 millimolar and decayed with a time constant of 1.2 milliseconds at cultured hippocampal synapses."

Measured: **both poles of the 44,000-fold span are NMDA-receptor-reported concentrations.** Herman's 25 nM is read off tonic NMDAR current; Clements' 1.1 mM is read off antagonist displacement from NMDARs. Neither is a bulk-extracellular-fluid measurement by any device. The four µM-class values, by contrast, are all bulk-ECF device measurements — a microdialysis probe or a glutamate-oxidase microelectrode array. `primary-source-supported` for what each instrument is.

Inferred by me: the 44,000-fold span is therefore a **receptor-domain** span — the concentration range an NMDAR behind the transporter screen experiences — being used to specify a **bulk-ECF surface device** that is not behind that screen. On that reading the four basal numbers are not four attempts at one quantity, the µM-class values are the correct design target for a surface aptamer electrode, and the span such a device must actually cover has never been measured, because no bulk-ECF transient amplitude exists in the ledger. `hypothesis`.

This is the sharpest available statement of why the flagship comparison fails, and it is independent of who is right about basal glutamate: even if Herman's 25 nM is the correct receptor-domain baseline and Clements' 1.1 mM the correct receptor-domain peak, both belong to a compartment the sensor does not sample.
...
**Measured** (by others; I inspected Hershey §3.1, Hascup 2010 §3.1–3.2, Hascup 2008 Table 1, and the Herman and Clements abstracts myself this pass): basal extracellular glutamate at 25 nM (slice, tonic NMDAR current), 3.3 ± 1.0 / 5.0 ± 1.2 µM (awake mouse MEA), 9.4 ± 0.6 µM (rat cortex dialysis, `Ed`-corrected, n = 11), 34.7 ± 11.8 µM (awake rat PFC MEA, n = 41); Gln 179 ± 20 µM in the same animals as the 9.4 µM; −40.8 ± 10.8% TTX effect on awake rat PFC resting glutamate (n = 7); that both Herman's 25 nM and Clements' 1.1 mM are NMDAR-reported rather than device-sampled concentrations; Hu's reported calibration windows and protocol clocks; that Hu fitted Langmuir–Freundlich and did not report n.
